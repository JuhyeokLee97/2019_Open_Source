...
int getEmptyCells(struct Pos empty[], struct Square m[][SIZE]) { //빈셀의 개수와 좌표를 얻기위한 함수
	int i, j; //for문 index
	int numOfEmptyCells = 0; //빈 셀의 개수를 저장할 변수
	for (i = 0; i < SIZE; i++)
		for (j = 0; j < SIZE; j++) // 모든 셀을 방문하는 for문
			if (!m[i][j].value) { //셀의 value값이 0인 경우
				...
				empty[numOfEmptyCells] = temp; //empty배열에 temp값을 넣고 n에 1을 더해준다
				numOfEmptyCells++;
			}
	return numOfEmptyCells; //빈 셀의 개수를 리턴
}
...
void addRandomSquare(struct Square m[][SIZE]) {
	...
	int numOfEmptyCells = getEmptyCells(empty, m); //value가 1인 cell의 개수를 불러오며, empty배열에 빈 셀들의 좌표를 저장한다
	if (numOfEmptyCells > 0) { // 빈 셀이 있다면
		random = generateRandom(0, n); //0과 n사이의 랜덤 수 저장
		struct Pos temp;
		temp.x = empty[random].x;
		temp.y = empty[random].y;
		m[temp.x][temp.y].value = 2; //빈 셀들 중 하나를 랜덤으로 골라 그 셀의 value를 2로 변경
	}
}
